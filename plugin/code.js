...
function setupVariables(fontStyles) {
  ...
  for (var i = 0; i < cols.length; i++) {
    if (cols[i].name === COLLECTION_NAME || cols[i].name === 'Font Size') {
      col = cols[i];
      if (col.name !== COLLECTION_NAME) col.name = COLLECTION_NAME;
      break;
    }
  }
  if (!col) {
  ...
  for (var mi = 0; mi < MODES.length; mi++) {
    if (!modeMap[MODES[mi]]) throw new Error('Thieu mode ' + MODES[mi]);
  }
...
async function bindVariables(frame, varMap) {
  ...
  for (var i = 0; i < nodes.length; i++) {
    ...
    if (varMap['font-family/arial']) {
      try { node.setBoundVariable('fontFamily', varMap['font-family/arial']); stats.family++; } catch(e) {}
    }
    if (sty) {
      var stk = 'font-style/' + sty.toLowerCase().replace(/\s+/g, '-');
      if (varMap[stk]) {
        try { node.setBoundVariable('fontStyle', varMap[stk]); stats.style++; } catch(e) {}
      }
    }
  }
  return stats;
...
figma.ui.onmessage = async function(msg) {
  ...
  // Standardize
  if (msg.standardize) {
    var fontStyles = collectFontStyles(frame);
    var res;
    try { res = setupVariables(fontStyles); }
    ...
    var stats = await bindVariables(frame, res.varMap);
    lines.push('✓ Bound ' + stats.size + ' nodes (lh:' + stats.lh + ' ls:' + stats.ls + ' fam:' + stats.family + ' sty:' + stats.style + ')');
  }

  // Duplicate
  if (msg.duplicate && msg.breakpoints.length > 0) {
    var res2 = res || (function() {
      var fs = collectFontStyles(frame);
      return setupVariables(fs);
    })();
    var cloned = duplicateFrames(frame, res2.col, res2.modeMap, msg.breakpoints);
